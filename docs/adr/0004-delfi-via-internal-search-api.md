...
- **Unsafe only at the end** — `+ - ! :` bind to the term that follows them, so a query ending on one is a parse error. Mid-query they are ordinary text, and over-stripping costs real hits (Delfi finds `Jean-Paul Sartre` but returns nothing for `Jean Paul Sartre`), so the trim at the end is per character rather than blanket:
  - A trailing token made of *nothing but* these operators is always dropped — `Hobit -`, `Hobit :` and `Hobit ++` all answer 500.
  - A trailing `:` or `!` is dropped even when attached to a word: `Hobit:` and `Hobit!` are parse errors too, and titles ending in `!` are common in Serbian.
  - A trailing `-` or `+` attached to a word is *kept*: `Hobit-` and `Hobit+` answer 200, and trimming them costs real hits — `C++` returns 15 products where `C` returns none.
  - Mid-query `:` costs no recall on this backend either, so it is left alone there — probing six real Delfi-stocked titles containing `:` returned the same result count with and without the colon (1 vs 1 each). Only a trailing or word-final `:` is a parse error; it does *not* behave like a Lucene field query here.
