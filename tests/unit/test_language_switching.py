...
def test_home_page_defaults_to_english() -> None:
    response = client.get("/")

    assert "Browse by genre" in response.text


def test_home_page_shows_serbian_latin_when_requested() -> None:
    response = client.get("/?lang=sr-latn")

    assert "Pregled po žanru" in response.text


def test_home_page_shows_transliterated_cyrillic_when_requested() -> None:
    response = client.get("/?lang=sr-cyrl")

    assert "Претрага" in response.text


def test_valid_lang_param_sets_a_cookie() -> None:
    response = client.get("/?lang=sr-latn")

    assert response.cookies.get("lang") == "sr-latn"


def test_cookie_is_honored_on_a_later_request_with_no_param() -> None:
...
def test_genre_labels_on_home_page_are_translated() -> None:
    response = client.get("/?lang=sr-latn")
